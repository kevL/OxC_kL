...
void SaveConverter::loadDatLIGlob()
{
	...
	for (size_t i = 0x04; i < 0x04 + 0x30; i += 4)
	{
		...
		_save->getExpenditureList().push_back(expenditure);
	}
...
void SaveConverter::loadDatMissions()
{
			...
			++i)
	{
		...
		if (wave != 0xFFFF)
		{
			...
			YAML::Node node;
			AlienMission *m = new AlienMission(
											*_rule->getAlienMission(_idMissions[mission]),
											*_save);
