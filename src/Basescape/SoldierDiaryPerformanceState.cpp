...
void SoldierDiaryPerformanceState::init()
{
	...
	if (_base == NULL)
	{
		...
		if (_soldierDead->getDiary() == NULL) // TEST
		{
			Log(LOG_INFO) << ". no Diary";
			return; // kL
		}

//		int kill = _soldierDead->getDiary()->getKillTotal(); // TEST
//		int stun = _soldierDead->getDiary()->getStunTotal(); // TEST
		//Log(LOG_INFO) << "performanceState: kill = " << kill << " stun = " << stun;
		...
		_lstMissionTotals->addRow(
							4, tr("STR_MISSIONS").arg(_soldierDead->getDiary()->getMissionTotal()).c_str(),
	...
	else
	{
		...
		_lstMissionTotals->addRow(
							4, tr("STR_MISSIONS").arg(_soldier->getDiary()->getMissionTotal()).c_str(),
...
void SoldierDiaryPerformanceState::drawSprites()
{
			...
			++i)
	{
		...
		if (vectorIterator < scrollDepth
			|| vectorIterator - scrollDepth >= static_cast<int>(_commendations.size()))
		{
			vectorIterator++;
