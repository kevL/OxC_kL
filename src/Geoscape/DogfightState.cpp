...
//	if (_ufo->isCrashed() == false && _ufo->getStatus() == Ufo::FLYING)
	{
		_ufoBreakingOff = false;
//		_craft->setSpeed(_ufo->getSpeed()); // -> let the movingTarget logic dictate speed
	}
...
//						ufoSize = 4;
				...
				if ((*i)->getGlobalType() == CWPGT_MISSILE) // Check if projectile passed its maximum range.
				{
			...
			else if ((*i)->getDirection() == D_DOWN) // Projectiles fired by UFO.
			{
				if ((*i)->getGlobalType() == CWPGT_MISSILE
					|| ((*i)->getGlobalType() == CWPGT_BEAM
						&& (*i)->toBeRemoved() == true))
				{
					hitprob = (*i)->getAccuracy();
		...
		if (_ufo->isCrashed() == true) // End dogfight if UFO is crashed or destroyed.
		{
			...
			if (RNG::percent(4 + (_diff * 4)) == true)						// Check retaliation trigger.
			{
														...
														alm_RETAL) == false)
				{
					...
					mission->start(mission->getRules().getWave(0).spawnTimer); // fix delay for first scout
