										...
										// kL_begin:
										{
													...
													++part)
											{
												...
												if (tile->getMapData(part))
												{
													...
													if (tile->getMapData(part)->getObjectType() == MapData::O_WESTWALL)
													{
...
//														break;
														...
														&& (tile->getMapData(MapData::O_OBJECT)->getBigWall() == 1			// bigBlock
															|| tile->getMapData(MapData::O_OBJECT)->getBigWall() == 4))	// westBlock
													{
...
//kL											tile->setDiscovered(true, 1);
										// kL_begin:
										{
													...
													++part)
											{
												...
												{
													//Log(LOG_INFO) << ". getMapData(part) VALID";
													if (tile->getMapData(part)->getObjectType() == MapData::O_NORTHWALL)
													{
...
//														break;
														...
														&& tile->getMapData(MapData::O_OBJECT)
														&& (tile->getMapData(MapData::O_OBJECT)->getBigWall() == 1		// bigBlock
															|| tile->getMapData(MapData::O_OBJECT)->getBigWall() == 5))	// northBlock
													{
				...
				te += 3)
		{
									...
									// the idea is to show an explosion animation but affect only that one tile.
			{
				...
				if (r > 0.5) // don't block epicentrum.
				{
					...
					if (_powerE < 1)
					{
						//Log(LOG_INFO) << ". _powerT < 1 BREAK[hori] " << Position(tileX, tileY, tileZ);
						break;
					}
					...
					if (origin->getPosition().z != tileZ) // up/down explosion decrease
					{
						...
						if (_powerE < 1)
						{
							//Log(LOG_INFO) << ". _powerT < 1 BREAK[vert] " << Position(tileX, tileY, tileZ);
							break;
						}
					}
 ...
 * @return, (int)block	-99 special case for Content & bigWalls to block vision and still get revealed, and for invalid tiles also
...
//							+ blockage(_save->getTile(startTile->getPosition() + tileEast), MapData::O_OBJECT, type, 6)) / 2;
//				else
//					block = (blockage(_save->getTile(startTile->getPosition() + tileNorth), MapData::O_OBJECT, type, 4)
//							+ blockage(_save->getTile(startTile->getPosition() + tileEast), MapData::O_OBJECT, type, 6)) < 510? 0: 255;
			}
		break;
		case 2: // east
			block = blockage(
...
//				if (type == DT_HE)
//					block += (blockage(_save->getTile(startTile->getPosition() + tileSouth), MapData::O_OBJECT, type, 0) +
//							blockage(_save->getTile(startTile->getPosition() + tileWest), MapData::O_OBJECT, type, 2)) / 2;
//				else
//					block += (blockage(_save->getTile(startTile->getPosition() + tileSouth), MapData::O_OBJECT, type, 0) +
//							blockage(_save->getTile(startTile->getPosition() + tileWest), MapData::O_OBJECT, type, 2)) < 510? 0: 255;
			}
		break;
		case 6: // west
			block = blockage(
...
/*		if (block > -1 // block > -99
			&& blockage(
				...
				< 0)
		{
			//Log(LOG_INFO) << "TileEngine::horizontalBlockage() EXIT, ret = -99";
			if (type == DT_NONE)
				return -1;	// special case for calculateFOV() to not decrement last tile from the trajectory.
...
//				&& type != DT_NONE
			{
				if (bigWall == Pathfinding::BIGWALL_NONE) // if (Content-part == true)
 ...
 * This is where the actual destruction takes place, and it
 * affects 7 objects (6 box sides and the object inside).
