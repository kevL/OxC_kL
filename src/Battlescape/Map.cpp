					...
					&& screenPosition.y < surface->getHeight() + _spriteHeight)
				{
										...
										++i)
								{
									...
									if (tmpSurface)
									{
											...
											&& _save->getTileEngine()->isVoxelVisible(voxelPos))
										{
											...
											tmpSurface->blitNShade(
													surface,
													bulletPositionScreen.x,
													bulletPositionScreen.y,
													16);
										}
						...
						&& _previewSetting %2)
					{
						...
						if (tmpSurface)
						{
							tmpSurface->blitNShade(
									surface,
									screenPosition.x - 16,
									screenPosition.y - (20 - tile->getTerrainLevel()),
									0,
									false,
									tileColor);
						}
						...
						&& !_save->getBattleState()->getMouseOverIcons())
					{
						if (_camera->getViewLevel() == itZ)
						{
							...
							// Wb_begin: 140125, UFOExtender Accuracy.
							if (_cursorType == CT_AIM
								&& Options::getBool("battleUFOExtenderAccuracy"))
							{
	...
	if (_explosionInFOV) // check if we got big explosions
	{
				...
				++i)
		{
			...
			else if ((*i)->isHit())
			{
				...
				tmpSurface->blitNShade(
						surface,
						bulletPositionScreen.x - 15,
						bulletPositionScreen.y - 15,
						0);
			}
			else
			{
