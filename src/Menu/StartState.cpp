	...
	switch (_load)
	{
		case LOADING_STARTED:
			try
			{
				...
				if (Options::getString("language").empty()) // No language set, detect based on system
				{
				...
				if (Options::getBool("playIntro")
					&& CrossPlatform::fileExists(introFile)
					&& (CrossPlatform::fileExists(introSoundFileDOS)
						|| CrossPlatform::fileExists(introSoundFileWin)))
				{
